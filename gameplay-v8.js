(() => {
  ...
  canvas.id = "gameplayV8Canvas";
  canvas.setAttribute("aria-label", "Predicted arena with mouse aiming and explosion effects");
  ...
  app.appendChild(rolePanel);

  const aimHud = document.createElement("div");
  aimHud.id = "aimHud";
  aimHud.className = "aim-hud";
  aimHud.dataset.active = "false";
  aimHud.innerHTML = `<span class="aim-hud-icon">⌖</span><div><strong id="aimHudTitle">AIM LOCKED</strong><small id="aimHudDetail">Hold right click and move the mouse</small></div>`;
  ...
  function cameraTransform(dt) {
    ...
    const lookAhead = aim.active ? 580 : 320;
  ...
  function updateAimHud() {
    ...
    document.getElementById("aimHudDetail").textContent = aim.active ? "Move mouse · release right click to lock" : "Hold right click anywhere in the arena";
  }
  ...
  WebSocket.prototype.send = function gameplayV8Send(payload) {
    let outgoing = payload;
    if (typeof payload === "string") {
      try {
        const message = JSON.parse(payload);
        if (["register_student", "reconnect_student", "input", "set_ready", "select_team"].includes(message.type)) observeSocket(this);
        if (message.type === "input") {
          input.dx = clamp(Number(message.dx) || 0, -1, 1);
          input.dy = clamp(Number(message.dy) || 0, -1, 1);
          input.dash = Boolean(message.dash);
          input.shoot = spacePressed;
  ...
  function spawnExplosion(player) {
    ...
    for (let index = 0; index < 34; index += 1) {
      ...
      const speed = 420 + Math.random() * 1450;
      particles.push({
        x: Number(player.x) || 0,
        y: Number(player.y) || 0,
        ...
        age: 0,
        color: Math.random() < 0.26 ? "#ffffff" : color
      });
    }
    explosions.push({ x: Number(player.x) || 0, y: Number(player.y) || 0, color, age: 0, duration: 0.95, particles });
    cameraShake = Math.max(cameraShake, player.id === state.playerId ? 28 : 13);
  ...
  function handleMessage(message) {
    ...
    if (message.type === "life_lost") showPickupToast(`${message.lives} LIFE${message.lives === 1 ? "" : "S"} REMAINING`, true);
  }
  ...
  function updateHud() {
    ...
    cohesionBadge.querySelector("small").textContent = volley === 3 ? "Full three-player formation active" : volley === 2 ? "Two allied players in formation" : "Move near teammates for ×2 or ×3";
  ...
  function drawPickup(pickup, t, frameAt) {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.roundRect(-size, -size, size * 2, size * 2, 6); ctx.fill(); ctx.stroke();
    ctx.shadowBlur = 0;
  ...
  function drawPlayer(player, t) {
    ...
    ctx.save();
    ctx.translate(x, y);
    ctx.globalAlpha = player.connected === false ? 0.35 : player.alive ? 1 : 0.16;
  ...
  function drawAim(t, frameAt) {
    ...
    ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(-arrowSize * 1.8, arrowSize); ctx.lineTo(-arrowSize * 1.8, -arrowSize); ctx.closePath(); ctx.fill();
    ctx.restore();
    ...
    ctx.beginPath(); ctx.arc(crossX, crossY, aim.active ? 15 : 11, 0, Math.PI * 2); ctx.moveTo(crossX - 22, crossY); ctx.lineTo(crossX + 22, crossY); ctx.moveTo(crossX, crossY - 22); ctx.lineTo(crossX, crossY + 22); ctx.stroke();
    ctx.restore();
  }
  ...
  function drawExplosions(t, dt) {
    ...
    for (let index = explosions.length - 1; index >= 0; index -= 1) {
      ...
      ctx.strokeStyle = rgba(explosion.color, (1 - progress) * 0.8);
      ctx.lineWidth = Math.max(1, 8 * (1 - progress));
      ctx.beginPath(); ctx.arc(centerX, centerY, (32 + progress * 260) * t.scale, 0, Math.PI * 2); ctx.stroke();
      ...
      for (const particle of explosion.particles) {
        ...
        ctx.beginPath(); ctx.arc(x, y, Math.max(1.4, particle.size * t.scale * alpha), 0, Math.PI * 2); ctx.fill();
      }
