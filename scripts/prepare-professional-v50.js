...
const partsDir=path.join(__dirname,"v50-payload");
const encoded=fs.readdirSync(partsDir).filter(name=>/^part-\d+\.txt$/.test(name)).sort().map(name=>fs.readFileSync(path.join(partsDir,name),"utf8").trim()).join("");
