...
]) assert.doesNotMatch(studentHtml, new RegExp(removedLayer.replaceAll(".", "\\.")), `${removedLayer} must not load on the v22 student page`);
...
for (const marker of [
  ...
  "gameplay-v9-plus-conversation-assets",
  ...
  "function drawOffscreenTeammates",
...
assert.match(gameplayCss, /three-student-role-panel-v22/);
assert.match(gameplayCss, /pickup-legend-v22/);
...
assert.match(studentHtml, /HOLD RIGHT CLICK on the arena to aim/);
assert.match(studentHtml, /Spacebar remains the only shooting control/);
assert.match(studentHtml, /Connection architecture preserved/);
...
console.log("Smoke test passed: Recovered Arena v22 restores prior visual mechanics and assets while preserving stable networking, Reporting v18, Geometry v19 and the authoritative v20 projectile server.");
